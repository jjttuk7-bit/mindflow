...
export async function describeImage(base64: string, mimeType: string): Promise<string> {
  ...
  const result = await model.generateContent([
    ...
    `이미지를 분석하고 적절한 캡션을 생성하세요.

이미지 유형별 캡션 전략:

1. 손글씨/짧은 텍스트/숫자 (메모, 포스트잇, 화이트보드 낙서 등):
   → 적힌 내용을 그대로 캡션으로 (예: "내일 3시 미팅", "우유, 달걀, 빵")

2. 책 표지/긴 글이 포함된 이미지 (책, 문서, 기사 캡처 등):
   → 제목 또는 핵심 내용만 캡션 (예: "클린 코드 - 로버트 마틴", "매출 보고서 Q3 요약")

3. 영수증/메뉴판/가격표:
   → 장소명 + 총액 또는 핵심 항목 (예: "스타벅스 12,500원", "이탈리안 레스토랑 메뉴")

4. 코드/에러 로그:
   → 핵심 에러 또는 코드 내용 요약 (예: "TypeError: undefined is not a function", "React useEffect 훅")

5. 일반 사진 (풍경, 음식, 사람, 물건 등):
   → 무엇이 찍혔는지 간결 설명 (예: "한강 야경", "파스타와 와인")

규칙:
- "이 이미지는~", "사진입니다" 같은 메타 설명 금지
- 최대 100자
- 한국어로 응답
- 캡션만 반환. 다른 텍스트 없이.`,
  ])
